...
def test_load_from_yaml(tmp_path, monkeypatch):
    ...
    assert cfg.gauntlex.attack_count == 5
...
def test_reports_dir_anchored_to_project_root_not_invoking_cwd(tmp_path, monkeypatch):
    """`gauntlex dashboard`/`gauntlex status` must resolve the same reports_dir as
    `gauntlex run` regardless of which subdirectory of the project they're invoked
    from — otherwise a dashboard launched from a different terminal/cwd silently
    reads an empty, unrelated .gauntlex/reports."""
    project_root = tmp_path / "project"
    subdir = project_root / "some" / "nested" / "dir"
    subdir.mkdir(parents=True)
    (project_root / ".gauntlex.yml").write_text("version: 1\n")

    monkeypatch.chdir(subdir)
    cfg = AppConfig.load()
    assert cfg.reports_dir == project_root / ".gauntlex" / "reports"


def test_reports_dir_anchored_at_project_root_itself(tmp_path):
    (tmp_path / ".gauntlex.yml").write_text("version: 1\n")
    cfg = AppConfig.load(tmp_path / ".gauntlex.yml")
    assert cfg.reports_dir == tmp_path / ".gauntlex" / "reports"


def test_explicit_deployment_wins_over_stale_api_key(tmp_path, monkeypatch):
    """A user's explicit `model_provider: local` in .gauntlex.yml must not be
...
def test_model_provider_env_wins_over_yaml(tmp_path, monkeypatch):
    """MODEL_PROVIDER (written by `gauntlex setup`) is the user's most recent
    explicit choice and always takes precedence over .gauntlex.yml."""
...
def test_zero_config_still_autodetects_from_api_key(tmp_path, monkeypatch):
    """With no .gauntlex.yml and no MODEL_PROVIDER, fall back to detecting the
    provider from whichever API key is present (zero-config / CI convenience)."""
    # `gauntlex.cli` calls load_dotenv() at import time, which can leak this
...
def test_load_remembers_project_after_finding_via_cwd(tmp_path, monkeypatch):
    ...
    project.mkdir()
    (project / ".gauntlex.yml").write_text("version: 1\n")
    monkeypatch.chdir(project)

    AppConfig.load()

    # isolated_home (conftest.py) points $HOME at this same tmp_path.
    last_project_file = tmp_path / ".gauntlex" / "last_project"
...
def test_explicit_config_wins_over_remembered_project(tmp_path, monkeypatch):
    ...
    (explicit_project / ".gauntlex.yml").write_text("version: 1\n")

    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)

    cfg = AppConfig.load(explicit_project / ".gauntlex.yml")
    assert cfg.config_source == (explicit_project / ".gauntlex.yml").resolve()
