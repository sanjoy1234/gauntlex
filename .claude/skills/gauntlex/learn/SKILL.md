---
name: gauntlex:learn
description: Ingest a completed GAUNTLEX run into the Knowledge Forge and update the Adaptive Brain's attack effectiveness scores. Call after reviewing a completed run to reinforce learning.
---
...
```
/gauntlex:learn --run-id <run_id> --outcome pass|fail
```

## Execution

```bash
gauntlex learn --run-id "$RUN_ID" --outcome "$OUTCOME"
```
