---
name: combatpair:report
description: Render a stored COMBATPAIR Resilience Report as Markdown, HTML, or structured JSON. Can attach to a GitHub PR comment.
---
...
```
/combatpair:report --run-id <id> [--format md|html|json] [--post-pr]
```

## Execution

```bash
combatpair report --run-id "$RUN_ID" --format "$FORMAT"
```
...
For `--format html`: Save to `.combatpair/reports/<run_id>.html` and show path.
