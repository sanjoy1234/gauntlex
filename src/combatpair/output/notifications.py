...
def send_slack(
    ...
    payload = {
        "attachments": [
            {
                "color": color,
                "fallback": f"COMBATPAIR ARS {ars:.2f} — {verdict}",
                "title": f"🛡️ COMBATPAIR Adversarial Gate — {verdict}",
                "title_link": "",
                "fields": [
                ...
                "footer": "COMBATPAIR Adversarial Co-Generation Engine",
            }
        ]
    }
...
def create_jira_ticket(
    ...
    summary = f"[COMBATPAIR] ARS {ars:.2f} gate failure — run {run_id[:16]}"
    description = (
        f"COMBATPAIR adversarial gate blocked a run with ARS {ars:.2f}.\n\n"
        f"Run ID: {run_id}\n"
        f"Missed attacks: {miss_count}\n\n"
        f"*Unmitigated vulnerabilities:*\n{miss_items}\n\n"
        f"Fix these security issues and re-run COMBATPAIR before merging."
    )
    ...
    payload = {
        "fields": {
            ...
            "labels": ["security", "combatpair", "adversarial-gate"],
        }
    }
