"""
COMBATPAIR MCP Server — expose COMBATPAIR as a Model Context Protocol (MCP) server.

Supports two transports:
  stdio  — local IDE integration (Claude Code, Cursor, Windsurf, Zed)
           Launch: combatpair mcp-server
  ...
  http   — team/enterprise deployment, served alongside the Combat Dashboard
           Launch: combatpair serve --mcp  (POST /mcp)

Protocol: JSON-RPC 2.0 / MCP 2024-11-05

Tools exposed:
  combatpair_run         — start adversarial assessment; returns run_id immediately
  ...
  combatpair_verify      — verify SHA-256 integrity of a stored Resilience Report

Async run dispatch:
  combatpair_run fires an asyncio.Task and returns in <1s.
  The event loop continues processing new messages (combatpair_status polls)
  while the engine runs in the background.

Environment variables (never committed — set in shell or .env):
  COMBATPAIR_MCP_PORT — HTTP port for --mcp mode (default: 8080)
"""
...
_TOOLS = [
    {
        "name": "combatpair_run",
        "description": (
            "Start a COMBATPAIR adversarial resilience assessment on a specification. "
            "Returns run_id immediately — the engine runs as a background task. "
            "Poll with combatpair_status to retrieve results. "
            ...
            "properties": {
                ...
                "domain": {
                    ...
                    "description": (
                        ...
                        "Use combatpair_policy_list to see all options."
                    ),
                },
                "language": {
                    "type": "string",
    ...
    {
        "name": "combatpair_status",
        "description": (
            "Poll for the results of a running COMBATPAIR adversarial assessment. "
        ...
        "inputSchema": {
            ...
            "properties": {
                "run_id": {
                    "type": "string",
                    "description": "The run_id returned by combatpair_run.",
                },
            },
            "required": ["run_id"],
        },
...
class MCPServer:
    """
    JSON-RPC 2.0 / MCP 1.0 server for COMBATPAIR.

    Maintains a dict of in-flight asyncio.Tasks (one per combatpair_run call) and
    their results. Background tasks run concurrently while the event loop processes
    new combatpair_status poll requests.
    ...
    def __init__(
        ...
        self._engine_fn: EngineFunction = engine_fn or _run_combatpair_engine
    ...
    async def _handle_tool_call(self, params: dict) -> dict:
        ...
        if name == "combatpair_run":
            return await self._tool_run(args)
        if name == "combatpair_status":
            return self._tool_status(args)
        if name == "combatpair_vault_stats":
            return self._tool_vault_stats(args)
        if name == "combatpair_policy_list":
            return self._tool_policy_list(args)
        if name == "combatpair_verify":
    ...
    async def _tool_run(self, args: dict) -> dict:
        ...
        run_id = f"combatpair-mcp-{uuid.uuid4().hex[:8]}"
        ...
        return {
            "content": [{
                ...
                "text": (
                    ...
                    f"domain: {domain}\n\n"
                    f"Poll for results:\n"
                    f"  combatpair_status(run_id='{run_id}')\n\n"
    ...
    def _tool_vault_stats(self, args: dict) -> dict:
        vault_dir = Path(".combatpair/vault")
        if not vault_dir.exists():
            return {
                "content": [{"type": "text", "text": "No Forge Ledger entries yet. Run a COMBATPAIR assessment to populate the vault."}],
    ...
    def _tool_policy_list(self, args: dict) -> dict:
        ...
        lines = ["Available COMBATPAIR security domain playbooks:", ""]
        for name, n, desc in domains:
            lines.append(f"  {name:<25} {n:>2} scenarios  {desc}")
        lines += ["", "Usage: combatpair_run(spec='...', domain='hipaa')"]
...
async def _run_combatpair_engine(
    run_id: str,
    spec: str,
    mode: str,
    domain: str,
    ...
    """
    Run the COMBATPAIR CombatPair engine on an inline spec and return a result dict.

    This is the MCP-facing entry point — equivalent to `combatpair run` but:
