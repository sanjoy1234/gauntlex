"""
Enterprise RBAC — GitHub team-based role enforcement for COMBATPAIR CPaaS.
...
class RbacConfig:
    ...
    admin_teams: list[str] = field(default_factory=lambda: _split_env("COMBATPAIR_ADMIN_TEAMS"))
    reviewer_teams: list[str] = field(default_factory=lambda: _split_env("COMBATPAIR_REVIEWER_TEAMS"))
    dev_teams: list[str] = field(default_factory=lambda: _split_env("COMBATPAIR_DEV_TEAMS"))
    allow_any_authenticated: bool = field(
        default_factory=lambda: os.environ.get("COMBATPAIR_ALLOW_ANY_AUTH", "true").lower() == "true"
    )
