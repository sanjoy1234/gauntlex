"""COMBATPAIR GitHub App webhook handler — CPaaS mode."""
...
async def handle_webhook(body: bytes, headers: dict, config: ServiceConfig) -> dict:
    ...
    3. Run COMBATPAIR in the configured mode
    ...
    from combatpair.harness.commands.run import execute as run_execute

    try:
        result = await run_execute(
            spec=spec,
            mode=config.combatpair_mode,
            config_path=config.combatpair_config_path,
        )
...
async def _post_pr_comment(
    ...
    from combatpair.output.forge_bot import render_pr_comment
...
def create_server(config: ServiceConfig | None = None):
    ...
    Usage:
        uvicorn combatpair.service.github_app:app --port 8080
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run: pip install combatpair-ai[serve]"
        )


# ASGI app entry point for uvicorn
app = None  # populated lazily on first import with COMBATPAIR_SERVE=1
if os.environ.get("COMBATPAIR_SERVE"):
    app = create_server()
