"""
GAUNTLEX CLI — entry point for all slash commands.
...
def _render_live_panel(state: _RunProgress) -> Panel:
    ...
    header = (
        f"[bold cyan]⚔  GAUNTLEX[/bold cyan]  [dim]{state.run_id}[/dim]\n"
...
@click.group()
@click.version_option(package_name="gauntlex-ai")
def main():
...
def run(issue: str, mode: str, domain: str, intent: str | None, pretty: bool,
    ...
    Primary (greenfield — spec-driven):
      gauntlex run --issue SPEC.md --pretty
      gauntlex run --issue SPEC.md --intent PROJ-123 --domain hipaa
      gauntlex run --issue SPEC.md --mode thorough --domain owasp_top10,finra
      gauntlex run --issue https://github.com/owner/repo/issues/42

    \b
    Secondary (brownfield — extract spec from existing repo):
      gauntlex run --issue https://github.com/pallets/flask --background
      gauntlex run --issue /path/to/local/repo --mode quick
    """
    ...
    if _missing:
        console.print()
        console.print("[red bold]⚔  GAUNTLEX cannot run — configuration is incomplete:[/red bold]")
        console.print()
        for problem, fix in _missing:
            console.print(f"  [red]✗[/red]  {problem}")
            console.print(f"     [dim]Fix: [bold cyan]{fix}[/bold cyan][/dim]")
        console.print()
        console.print("  [dim]Run [bold cyan]gauntlex setup[/bold cyan] for the full wizard.[/dim]")
        console.print()
        sys.exit(1)
    ...
    if background:
        ...
        # of whether `gauntlex` is on PATH (editable installs, virtualenvs, etc.)
        entry = (
            "from gauntlex.cli import main; main(standalone_mode=True)"
        )
        ...
        console.print(f"\n[bold cyan]⚔  GAUNTLEX[/bold cyan]  run started in background")
        ...
        console.print(f"   Check  : [bold]gauntlex status[/bold]")
        console.print()
        return
...
async def _run_async(
    ...
    state = _RunProgress(
        ...
        issue_label=label,
        mode=mode,
        attack_total=cfg.gauntlex.attack_count,
    )
    ...
    if use_live:
        with Live(_render_live_panel(state), console=console,
            ...
            try:
                ...
                console.print("[dim]  Run [bold cyan]gauntlex setup --model[/bold cyan] to change provider or refresh API key[/dim]")
                sys.exit(1)
        ...
        try:
            ...
            _plain("  Fix: gauntlex setup --model   (change provider or refresh API key)")
            _status(status="error", error=error_msg)
            sys.exit(1)
...
def status_cmd(show_all: bool):
    ...
    if not rows:
        console.print("[dim]No runs found. Start one with:[/dim] [bold]gauntlex run --issue <url> --pretty[/bold]")
        return
    ...
    if running:
        console.print(f"\n[dim]{running} run(s) active — refresh with [bold]gauntlex status[/bold][/dim]")
    console.print()
...
def validate(spec: str | None, pretty: bool):
    ...
    cfg = AppConfig.load()
    checks.append(("Config loaded", True, str(Path(".gauntlex.yml").exists()) + " (.gauntlex.yml)"))
...
# ──────────────────────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────────────────────
# gauntlex setup  — interactive first-run wizard
# ──────────────────────────────────────────────────────────────────────────────
...
def setup(section_model: bool, section_tokens: bool):
    """Configure GAUNTLEX — run at any time to change model or refresh tokens.
...
def _run_setup_wizard(section_model: bool = False, section_tokens: bool = False) -> None:  # noqa: C901
    ...
    console.print()
    console.print("[bold blue]⚔  GAUNTLEX Setup Wizard[/bold blue]")
    if section_model:
    ...
    if run_all or section_model:
        ...
        choice = ""
        console.print("[dim]Model settings preserved. Run [bold cyan]gauntlex setup --model[/bold cyan] to change provider.[/dim]")
        console.print()
    ...
    if not _do_step3:
        console.print("[dim]Integration tokens preserved. Run [bold cyan]gauntlex setup --tokens[/bold cyan] to update.[/dim]")
        console.print()

    if _do_step3:
        console.print()
        console.print("[bold]Step 3 of 4 — Business intent integrations[/bold]")
        console.print("[dim]GAUNTLEX's attack surface = spec (what to build) + intent (why it's needed).[/dim]")
        console.print("[dim]Connect your issue tracker so GAUNTLEX automatically widens its attack surface[/dim]")
        console.print("[dim]using the business context from your tickets.[/dim]")
        console.print()
        console.print("  [dim]Usage after setup:  gauntlex run --issue SPEC.md --intent PROJ-123[/dim]")
        console.print()
    ...
    if _do_step3 and new_env.get("JIRA_URL") and new_env.get("JIRA_EMAIL") and new_env.get("JIRA_TOKEN"):
        ...
        console.print("  │  Usage: gauntlex run --issue SPEC.md --intent PROJ-123")
        ...
        if click.confirm("  Connect Jira?", default=True):
            ...
            try:
                ...
                console.print("  [dim]Credentials saved — verify manually with: gauntlex run --intent PROJ-1[/dim]")
        else:
    ...
    if _do_step3 and new_env.get("CONFLUENCE_URL") and new_env.get("CONFLUENCE_EMAIL") and new_env.get("CONFLUENCE_TOKEN"):
        ...
        console.print("  │  Usage: gauntlex run --issue SPEC.md --intent https://yourorg.atlassian.net/wiki/spaces/...")
    ...
    if _do_step3 and not new_env.get("AHA_DOMAIN"):
        ...
        console.print("  │  Usage: gauntlex run --issue SPEC.md --intent https://yourco.aha.io/features/FEAT-1")
        console.print("  └─────────────────────────────────────────────────────────────────")
    ...
    lines = [
        "# GAUNTLEX — written by `gauntlex setup`",
        "# Re-run `gauntlex setup` to change provider or add integrations.",
        "",
    ]
    ...
    console.print("    [bold cyan]gauntlex run --issue examples/demo_issue.md --pretty[/bold cyan]   ← first adversarial session")
    console.print("    [bold cyan]gauntlex integrate[/bold cyan]                                       ← wire into your IDE")
    console.print("    [bold cyan]gauntlex dashboard[/bold cyan]                                       ← open the web UI")
    console.print()
...
@click.option("--force", is_flag=True, help="Overwrite existing .gauntlex.yml")
def init(domain: str, force: bool):
    """Scaffold .gauntlex.yml with sensible defaults."""
    config_path = Path(".gauntlex.yml")
    if config_path.exists() and not force:
        console.print("[yellow]⚠[/yellow]  .gauntlex.yml already exists. Use --force to overwrite.")
        sys.exit(0)

    content = DEFAULT_CONFIG_YAML.replace("owasp_top10@2025.1", f"{domain}@2025.1")
    config_path.write_text(content)
    console.print(f"[green]✓[/green]  Created .gauntlex.yml (domain: {domain})")
    console.print("  Edit [bold]gate.minimum_ars[/bold] to set your quality bar (default 0.80).")
    console.print("  Run [bold]gauntlex validate[/bold] to confirm environment is ready.")
...
def stats(days: int, learning_curve: bool):
    ...
    console.print(f"\n[bold]GAUNTLEX Stats[/bold] · last {days} days\n")
...
def compare(run_id_a: str, run_id_b: str, pretty: bool):
    ...
    if pretty:
        ...
        console.print(f"\n[bold]GAUNTLEX Compare[/bold]  {run_id_a[:8]} → {run_id_b[:8]}")
...
def audit(days: int, domain: str | None):
    ...
    table = Table(show_header=True, header_style="bold", title=f"GAUNTLEX Compliance Audit — last {days}d")
...
# ──────────────────────────────────────────────────────────────────────────────
# gauntlex policy (subcommand group)
# ──────────────────────────────────────────────────────────────────────────────
...
@click.option("--policies-dir", default=".gauntlex/policies", show_default=True,
...
def policy_install(domain: str, force: bool, policies_dir: str):
    ...
    if result.already_installed:
        ...
        console.print(f"  Use: [bold]gauntlex run --domain {domain}[/bold]")
    else:
...
def policy_hub():
    ...
    table = Table(show_header=True, header_style="bold", title="GAUNTLEX Policy Hub")
    table.add_column("Domain", style="bold")
    table.add_column("Version")
    table.add_column("Framework")
    table.add_column("Scenarios", justify="right")
...
def policy_validate(domain: str):
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] Domain '{domain}' not found. Run [bold]gauntlex policy list[/bold].")
        sys.exit(1)
...
@click.option("--vault-dir", default=".gauntlex/vault", show_default=True,
...
@click.option("--config", "config_path", default=None,
              help="Path to .gauntlex.yml. You shouldn't need this — the dashboard "
...
def dashboard(port: int, host: str, reload: bool, config_path: str | None):
    ...
    if cfg.config_source:
        ...
        console.print(
            f"  [yellow]⚠  No GAUNTLEX project found — never seen from this directory, "
            f"and nothing remembered from a previous run.[/yellow]\n"
            f"     Reports will be read from: [dim]{cfg.reports_dir}[/dim] (probably empty)\n"
            f"     Fix: run any [bold]gauntlex[/bold] command inside your project once "
            f"(e.g. [bold]gauntlex status[/bold]), or launch with "
            f"[bold cyan]gauntlex dashboard --config /path/to/.gauntlex.yml[/bold cyan]"
        )
    console.print(f"  Stop: Ctrl+C\n")

    import uvicorn
    uvicorn.run(
        "gauntlex.dashboard.app:create_app",
        factory=True,
        host=host,
        port=port,
        reload=reload,
...
def serve(port: int, host: str, reload: bool, rbac: bool, config_path: str | None):
    """Start GAUNTLEX CPaaS (GitHub App webhook server)."""
    try:
        import uvicorn  # noqa: F401
    except ImportError:
        console.print("[red]Error:[/red] gauntlex serve requires FastAPI and Uvicorn.")
        console.print("Install with: [bold]pip install gauntlex-ai[ui][/bold]")
        sys.exit(1)

    import os
    gauntlex_cfg = AppConfig.load(config_path)
    if gauntlex_cfg.config_source:
        os.environ["GAUNTLEX_CONFIG_PATH"] = str(gauntlex_cfg.config_source)
    else:
        console.print(
            f"  [yellow]⚠  No GAUNTLEX project found — never seen from this directory, "
            f"and nothing remembered from a previous run.[/yellow]\n"
            f"     Reports will be read from: [dim]{gauntlex_cfg.reports_dir}[/dim] (probably empty)\n"
            f"     Fix: run any [bold]gauntlex[/bold] command inside your project once "
            f"(e.g. [bold]gauntlex status[/bold]), or launch with "
            f"[bold cyan]gauntlex serve --config /path/to/.gauntlex.yml[/bold cyan]\n"
        )
...
def forge_network_status():
    ...
    if cfg.enabled:
        ...
        console.print("  Enable with: GAUNTLEX_FORGE_NETWORK_ENABLED=true")
...
@click.option("--reports-dir", type=click.Path(), default=None,
              help="Directory of GAUNTLEX report JSON files named <agent>--<task>.json")
...
@click.option("--title", default="GAUNTLEX ARS Leaderboard", show_default=True,
              help="Page title")
...
@click.option("--config", "config_path", default=None, help="Path to .gauntlex.yml")
def mcp_server(config_path: str | None):
    """
    Start GAUNTLEX as an MCP server (stdio transport).

    Exposes GAUNTLEX's adversarial assessment capabilities to any
    MCP-compatible coding tool: Claude Code, Cursor, Windsurf, Zed, etc.

    \b
    Tools exposed:
      gauntlex_run         — start adversarial assessment (returns run_id)
      gauntlex_status      — poll for results by run_id
      gauntlex_vault_stats — Knowledge Forge statistics
      gauntlex_policy_list — list available security domains
...
# ──────────────────────────────────────────────────────────────────────────────
# gauntlex findings  — vulnerability-first view of last (or specified) run
# ──────────────────────────────────────────────────────────────────────────────
...
def findings(run_id: str | None, fmt: str):
    ...
    Examples:
      ...
      gauntlex findings gauntlex-2026-...  # specific run
    ...
    if run_id is None:
        ...
        if not last_id_file.exists():
            console.print("[red]No recent run found.[/red] Run [bold]gauntlex run --issue ...[/bold] first.")
            sys.exit(1)
...
# ──────────────────────────────────────────────────────────────────────────────
# gauntlex integrate  — one-command IDE / platform setup
# ──────────────────────────────────────────────────────────────────────────────
...
"""
    ...
    if not dry_run:
        ...
        console.print("\nRestart your IDE, then ask it to run [bold]gauntlex_run[/bold] on any spec.")
        console.print("Or: [bold]gauntlex run --issue examples/demo_issue.md --pretty[/bold]\n")
    else:
...
def _load_github_repo_spec(owner: str, repo: str) -> str | None:
    ...
    tmp = tempfile.mkdtemp(prefix="gauntlex-repo-")
    try:
...
def _print_run_summary(report: dict, passed: bool) -> None:
    ...
    if missed:
        ...
        console.print("  [dim]Retry with --mode quick, or run `gauntlex setup` to switch to a more reliable model.[/dim]")
    else:
    ...
    console.print(f"  Report: gauntlex report {report['run_id']} --format html")
    console.print(f"{'─'*52}\n")
